  ...
  if (hasPermission === null) {
    return (
      <View style={tailwind`flex-1 bg-black justify-center items-center`}>
        <Text style={tailwind`text-white text-lg`}>Requesting camera permission...</Text>
      </View>
    );
  }
  ...
  if (hasPermission === false) {
    return (
      <View style={tailwind`flex-1 bg-black justify-center items-center p-6`}>
        <Text style={tailwind`text-white text-lg text-center mb-4`}>
          Camera permission is required to scan QR codes
        </Text>
        <Pressable
          style={tailwind`bg-purple-600 px-6 py-3 rounded-xl`}
          onPress={onClose}
        >
